package main.java.me.avankziar.scc.general.objects;

public class StaticValues
{
	public final static String M2PS = "messagetoproxysingle";
	public final static String M2PM = "messagetoproxymultiple";
	public final static String M2PA = "messagetoproxyall";
	
	public final static String BC2BS = "basecomponenttobungeesingle";
	public final static String BC2BM = "basecomponenttobungeemultiple";
	public final static String BC2BA = "basecomponenttobungeeall";
	
	public final static String TM2BS = "titlemessagetobungeesingle";
	public final static String TM2BM = "titlemessagetobungeemultiple";
	public final static String TM2BA = "titlemessagetobungeeall";
	
	public final static String ABM2BS = "actionbarmessagetobungeesingle";
	public final static String ABM2BM = "actionbarmessagetobungeemultiple";
	public final static String ABM2BA = "actionbarmessagetobungeeall";
	
	public final static String SENDSOUND = "sendsound";
	
	public final static String SCC_TOPROXY = "scc:simplechatproxy";
	public final static String SCC_TOSPIGOT = "scc:simplechatspigot";
	public final static String SCC_TASK_LOCATIONUPDATE = "updatelocation";
	public final static String SCC_TASK_PINGAPLAYER = "pingaplayer";
	public final static String SCC_TASK_BROADCAST = "broadcast";
	public final static String SCC_TASK_W = "message_w";
	public final static String SCC_TASK_MYSQLPERFORMANCE = "mysqlperformance";
	public final static String SCC_TASK_ARG = "task_arg";
	public final static String SCC_EDITOR = "scceditor";
	public final static String SCC_REGISTERCHANNEL = "scc_registerchannel";
	public final static String SCC_REGISTERCHATTITLE = "scc_registerchattitle";
}
