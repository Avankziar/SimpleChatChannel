...
public class ARGPermanentChannel_Ban extends ArgumentModule
{
	...
	public void run(CommandSource sender, String[] args)
	{
		...
		if(cc.getCreator().equals(targetuuid) && cc.getVice().contains(player.getUniqueId().toString()))
		{
			...
			player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.ViceCannotBanCreator")));
			return;
		}
		if(targetuuid.equals(player.getUniqueId().toString()))
		{
			player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.OwnerCantSelfBan")));
			return;
		}
		if(cc.getBanned().contains(targetuuid))
		{
			player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.AlreadyBanned")));
			return;
		}
		...
		player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.YouHasBanned")
				.replace("%player%", target)));
		Optional<Player> t = plugin.getServer().getPlayer(target);
		if(t.isPresent())
		{
			t.get().sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.YourWereBanned")
