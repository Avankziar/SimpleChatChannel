...
public class ARGPermanentChannel_Ban extends ArgumentModule
{
	...
	public ARGPermanentChannel_Ban(SCC plugin, ArgumentConstructor argumentConstructor)
	{
	...
	public void run(CommandSender sender, String[] args)
	{
		...
		if(cc.getCreator().equals(targetuuid) && cc.getVice().contains(player.getUniqueId().toString()))
		{
			...
			player.sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.ViceCannotBanCreator")));
			return;
		}
		if(targetuuid.equals(player.getUniqueId().toString()))
		{
			player.sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.OwnerCantSelfBan")));
			return;
		}
		if(cc.getBanned().contains(targetuuid))
		{
			player.sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.AlreadyBanned")));
			return;
		}
		...
		player.sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.YouHasBanned")
				.replace("%player%", target)));
		ProxiedPlayer t = ProxyServer.getInstance().getPlayer(target);
		if(t != null)
		{
			t.sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PermanentChannel.Ban.YourWereBanned")
