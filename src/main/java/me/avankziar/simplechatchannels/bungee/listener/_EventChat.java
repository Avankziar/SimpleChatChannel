...
public class _EventChat implements Listener 
{
	private SimpleChatChannels plugin;
	HashMap<String,String> reply = new HashMap<String, String>();
	
	public _EventChat(SimpleChatChannels plugin)
	{
		this.plugin = plugin;
	}
	...
	public void onChat(ChatEvent event)
	{
		...
		{
			if(yamlHandler.getConfig().getBoolean("Denizen_Active", false))
			{
				return;
			} else
			{
		...
		String channelwithoutsymbol = yamlHandler.getLang().getString("ChannelSymbol.WithoutSymbol");
		String channel = yamlHandler.getChannel(channelwithoutsymbol, event.getMessage());
		String symbol = yamlHandler.getSymbol(channel);
		
		boolean timeofdays = yamlHandler.getConfig().getBoolean("AddingTimeOfDays", false);
		String timeofdaysformat = utility.getDate(yamlHandler.getConfig().getString("TimeOfDaysFormat"));
		String timeofdaysoutput = ChatApi.tl(yamlHandler.getConfig().getString("TimeOfDaysOutput")
				.replace("%time%", timeofdaysformat));
		
		if(channel.equalsIgnoreCase("Local"))
		{
			if(!yamlHandler.getConfig().getBoolean("Is_Scc_On_Spigot", false))
			{
				event.setCancelled(true);
				return;
			} else
		...
		if(!canchat)
		{
			...
			if(millitime==0)
			{
				...
				player.sendMessage(ChatApi.tctl(yamlHandler.getLang().getString("EventChat.Muted")
						.replace("%time%", time)));
				return;
			} else
			{
				...
				|| channel.equalsIgnoreCase("Event")) 
		{
			...
			if(utility.getWordfilter(event.getMessage().substring(symbol.length())))
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.Wordfilter")));
				return;
			}
			...
			if(event.getMessage().substring(symbol.length()).length()<0)
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.MessageToShort")));
				return;
			}
		...
		} else if(channel.equals("Temp")) //----------------------------------------------------------Temporärer Channel
		{
			...
			if(cc == null)
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.ChannelGeneral.NotInAChannel")));
				return;
			}
			...
			TextComponent channels = ChatApi.apiChat(yamlHandler.getLang().getString("Channels.Perma")
					.replace("%channel%", cc.getName()), 
					ClickEvent.Action.SUGGEST_COMMAND, symbol+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.Perma"));
			
			List<BaseComponent> prefix = utility.getPrefix(player);
			
			TextComponent playertext = ChatApi.apiChat(yamlHandler.getLang().getString("PlayerColor")+player.getName(), 
					ClickEvent.Action.SUGGEST_COMMAND, yamlHandler.getSymbol("PrivateMessage")+player.getName()+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.PrivateMessage")
		...
		} else if(channel.equals("Perma")) //----------------------------------------------------------Permanenter Channel
		{
			...
			if(cc == null)
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.PCSymbol.ChannelUnknown")));
				return;
			}
			...
			if(!cc.getMembers().contains(player.getUniqueId().toString()))
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("CmdScc.ChannelGeneral.NotInAChannelII")));
				return;
			}
		...
		} else if(channel.equalsIgnoreCase("Group")) //----------------------------------------------------------Gruppe Channel
		{
			...
			if(ps.equals("scc.no_prefix_suffix"))
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.NoSpace")));
				return;
			}
			
			TextComponent channels = ChatApi.apiChat(yamlHandler.getLang().getString("Channels.Group")
					.replace("%group%", preorsuffix), 
					ClickEvent.Action.SUGGEST_COMMAND, symbol+pors+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.Group"));
		...
		} else if(channel.equalsIgnoreCase("PrivateMessageRe")) //Reply Message
		{
			...
			if(!reply.containsKey(pl))
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.NoRePlayer")));
				return;
			}
			...
			if(ProxyServer.getInstance().getPlayer(UUID.fromString(target)) == null)
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("NoPlayerExist")));
				return;
			}
			...
			TextComponent channel1 = ChatApi.apiChat(yamlHandler.getLang().getString("Channels.PrivateMessage"), 
					ClickEvent.Action.SUGGEST_COMMAND, yamlHandler.getSymbol("PrivateMessage")+player.getName()+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.PrivateMessage")
					.replace("%player%", player.getName()));
			
			TextComponent channel2 = ChatApi.apiChat(yamlHandler.getLang().getString("Channels.PrivateMessage"), 
			...
			TextComponent player2text = ChatApi.apiChat(yamlHandler.getLang().getString("PlayerToPlayer")+tr.getName(), 
					ClickEvent.Action.SUGGEST_COMMAND, yamlHandler.getSymbol("PrivateMessage")+tr.getName()+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.PrivateMessage")
					.replace("%player%", player.getName()));
			...
			if(cut != null)
			{
				if(!cut.isChannelPrivateMessage())
				{
					...
					player.sendMessage(
							ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.PlayerNoPrivateMessage")));
			...
			if(utility.getIgnored(tr, player, true))
			{
				...
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("EventChat.PlayerIgnoreYou")));
				utility.spy(MSG1);
				player.sendMessage(MSG2);
				reply.put(pl, trl);
				return;
		...
		} else if(channel.equalsIgnoreCase("PrivateMessage")) //Private Message
		{
			...
			TextComponent channel1 = ChatApi.apiChat(
					yamlHandler.getLang().getString("Channels.PrivateMessage"), 
					ClickEvent.Action.SUGGEST_COMMAND, yamlHandler.getSymbol("PrivateMessage")+player.getName()+" ", 
					HoverEvent.Action.SHOW_TEXT, yamlHandler.getLang().getString("ChannelExtra.Hover.PrivateMessage")
					.replace("%player%", player.getName()));
