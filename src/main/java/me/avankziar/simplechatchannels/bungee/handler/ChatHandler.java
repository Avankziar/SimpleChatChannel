...
import main.java.me.avankziar.simplechatchannels.bungee.assistance.TimeHandler;
...
import main.java.me.avankziar.simplechatchannels.bungee.listener.ChatListener;
...
import main.java.me.avankziar.simplechatchannels.bungee.objects.chat.Components;
...
public class ChatHandler
{
	...
	public void startPrivateConsoleChat(CommandSender console, ProxiedPlayer other, String message)
	{
		...
		String basecolor = usedChannel.getInChatColorMessage();
	...
	public void sendBroadCast(CommandSender players, Channel usedChannel, String message)
	{
		...
		for(ProxiedPlayer all : plugin.getProxy().getPlayers())
		{
			...
			//ADDME play Ping, mit Bypassperm
		}
	}
	...
	private boolean preChecks(ProxiedPlayer player, Channel usedChannel, String message)
	{
				...
				&& ChatListener.spamMapII.get(player.getUniqueId().toString()).containsKey(usedChannel.getUniqueIdentifierName()))
		{
			...
			{
				player.sendMessage(ChatApi.tctl(plugin.getYamlHandler().getLang().getString("ChatListener.PleaseWaitALittleWithSameMessage")
						.replace("%channel%", usedChannel.getInChatName())
						.replace("%time%", TimeHandler.getDateTime(last))));
				return false;
			}
	...
	public Components getMessageParser(CommandSender players, String message, Channel usedChannel, String channelColor)
	{
		...
		for(String f : function)
		{
							...
							.getString("ChatReplacer.Emoji.End")))
			{
				...
				{
					TextComponent tc = ChatApi.tctl(plugin.getYamlHandler().getEmojis().getString(
							f.substring(plugin.getYamlHandler().getConfig().getString("ChatReplacer.Emoji.Start").length(),
									f.length()-plugin.getYamlHandler().getConfig().getString("ChatReplacer.Emoji.End").length())));
					components.addAllComponents(tc);
					continue;
				}				
			} else if(f.startsWith(plugin.getYamlHandler().getConfig()
